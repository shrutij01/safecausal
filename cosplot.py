...
def get_data(type="same"):
    if type == "same":
        ...
        values_scope = np.array(
            [0.4088, 0.6743, 0.7366, 0.3552, 0.7109, 0.7686]
        )
        std_devs_scope = np.array(
            [0.1067, 0.1170, 0.0919, 0.1074, 0.1542, 0.0596]
        )
        labels = [
...
def plot_udr():
    ...
    ax.plot(indices, udr_bin1_elev, color="orange", marker="o", linewidth=3)
    ax.errorbar(
        indices_2,
        mcc_bin1_fiv,
        yerr=std_bin1_fiv,
    ...
    #     linewidth=3,
    # )
    # ax.plot(indices, udr_bin2_elev, color="orange", marker="o", linewidth=3)
    # ax.errorbar(
    #     indices_2,
    #     mcc_bin2_thir,
    #     yerr=std_bin2_thir,
    ...
    ax.set_xlabel(
        r"{$\texttt{primal\_lr}, \beta$}", fontweight="bold"
    )  # Using LaTeX for x-label
    ax.set_ylabel("UDR/MCC", fontweight="bold", fontsize=23)
    ax.set_title(
        r"Comparison of UDR and MCC Values for \textsc{lang}(1, 1)",
        fontsize=25,
    )
    ...
    ax.annotate(
        "Optimal HPs",
        xy=(all_indices[4], udr_bin1_fiv[1]),
