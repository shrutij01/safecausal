...
def get_concept_detection_logits(
    z, encoder_weight, encoder_bias, decoder_bias, modeltype
):
    """
    ...
    print(
        f"entropy - max: {entropy.max()}, min: {entropy.min()}, mean: {entropy.mean()}"
    )
...
def main(args):
    ...
    if args.modeltype == "llamascope":
        _, decoder_bias, encoder_weight, encoder_bias = (
            load_llamascope_checkpoint()
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate concept detection accuracy"
    )
    parser.add_argument(
