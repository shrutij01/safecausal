#!/usr/bin/env python3
"""
Evaluate MCC (max Pearson correlation) and linear probe across datasets.
...
For each: computes MCC, steering cosine similarity, and optional linear probe.
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate MCC and linear probe across datasets"
    )
    parser.add_argument(
    ...
    for concept in concepts:
        ...
        if trained_sae_model is not None:
            if flat_indices is not None:
                c_trained_acts = trained_sae_activations[flat_indices]
            else:
                c_trained_acts = trained_sae_activations
        ...
        if pretrained_sae_model is not None:
            if flat_indices is not None:
                c_pretrained_acts = pretrained_sae_activations[flat_indices]
            else:
                c_pretrained_acts = pretrained_sae_activations
