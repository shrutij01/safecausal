...
embedding_files=(
    "/network/scratch/j/joshi.shruti/ssae/safearena/safearena_L_32_M_llama3.h5"
    "/network/scratch/j/joshi.shruti/ssae/safearena/safearena_L_14_M_llama3.h5"
)
data_configs=(
    "/network/scratch/j/joshi.shruti/ssae/safearena/safearena_32_config.yaml"
    "/network/scratch/j/joshi.shruti/ssae/safearena/safearena_14_config.yaml"
)
...
primal_lrs=(
    "--primal-lr 0.001" "--primal-lr 0.0005"
)
loss_types=(
    "--loss-type relative" "--loss-type absolute"
)
batch_sizes=(
    "--batch-size 64" "--batch-size 72" "--batch-size 84" "--batch-size 128"
)
norm_types=(
    "--norm-type ln"
)
...
# for the index-aligned embedding and config files
                        ...
                        for dual_lr in "${dual_lrs[@]}"; do
                            for epoch in "${epochs[@]}"; do
                                for seed in "${seeds[@]}"; do
                                    ...
                                    echo "python linear_sae.py --embeddings-file ${embedding_file} --data-config-file ${data_config} ${overcompleteness_factor} ${primal_lr} ${dual_lr} ${norm_type} ${loss_type} ${batch_size} ${epoch} ${seed}" >> "${script_name}"
