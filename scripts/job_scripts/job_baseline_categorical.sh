...
embedding_files=(
    "/network/scratch/j/joshi.shruti/ssae/categorical/categorical_embeddings_layer_32.h5"
)
data_configs=(
    "/network/scratch/j/joshi.shruti/ssae/categorical/categorical_32_config.yaml"
)
epochs=(
    "--num-epochs 1000"
)
                    ...
                    for batch_size in "${batch_sizes[@]}"; do
                        for epoch in "${epochs[@]}"; do
                            for seed in "${seeds[@]}"; do
                                ...
                                echo "python lin_baseline.py --embeddings-file ${embedding_file} --data-config-file ${data_config} ${primal_lr} ${batch_size} ${norm_type} ${indicator_threshold} ${epoch} ${seed}" >> "${script_name}"
